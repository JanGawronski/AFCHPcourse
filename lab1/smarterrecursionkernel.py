...
def smarter_recursion_with_kernel(G: list[list[int]]):
    ...
    for k in range(1, len(G)):
        (kernel_G, cover, vertex_reverse_mapping) = kernelization(G, k)
        edges = edgeList(kernel_G)
        res = VC(edges, [], k)
        if res is not None:
            return cover + [vertex_reverse_mapping[v] for v in res]
