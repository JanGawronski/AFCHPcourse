...
def set_order_solver(formula: list[list[int]]) -> tuple[dict[int, int] | str, int]:
    ...
    (result, calls) = set_order_solver(simplify_CNF(tail, evaluation))
...
if __name__ == "__main__":
    for ((size, formula), filename) in sorted([(loadCNF("sat/" + file.name), file.name) for file in scandir("sat")]):
        start = time() 
        result, calls = set_order_solver(formula)
