from os import scandir, setregid
from collections import Counter
from dimacs import loadCNF
from sat2cnf import sat2cnf
from time import time

def two_cnf_solver(formula: list[list[int]]) -> tuple[dict[int, int] | str, int]:
    (formula, evaluation1) = unit_propagation(formula)
    (formula, evaluation2) = fixed_sign_elimination(formula)
    ...
    if all(len(clause) == 2 for clause in formula):
        result = sat2cnf(formula, {})
        if result == u'UNSAT':
            return (u'UNSAT', 1)
        else:
            return(result | evaluation1 | evaluation2, 1)
       
    counter = Counter(abs(literal) for clause in formula for literal in clause)

    for clause in formula:
        clause.sort(key = lambda x: counter[abs(x)], reverse=True)

    formula.sort(key = lambda x: counter[abs(x[0])], reverse=True)
        
    head = formula[0]
    tail = formula[1:]

    evaluation = {head[0]: 1, -head[0]: -1} | evaluation1 | evaluation2

    (result, calls) = two_cnf_solver(simplify_CNF(tail, evaluation))
...
def fixed_sign_elimination(formula: list[list[int]]) -> tuple[list[list[int]], dict[int, int]]:
    if formula is None:
        return (None, {})

    fixed_sign_literals = set(literal for clause in formula for literal in clause) - set(-literal for clause in formula for literal in clause)
    
    if len(fixed_sign_literals) > 0:
        formula, evaluation = fixed_sign_elimination(simplify_CNF(formula, {clause: 1 for clause in fixed_sign_literals} | {-clause: -1 for clause in fixed_sign_literals}))
        return (formula, evaluation | {clause: 1 for clause in fixed_sign_literals} | {-clause: -1 for clause in fixed_sign_literals})
...
if __name__ == "__main__":
    for ((size, formula), filename) in sorted([(loadCNF("sat/" + file.name), file.name) for file in scandir("sat")]):
        start = time() 
        result, calls = two_cnf_solver(formula)
